...
//If supervisior Actually clicked register button
if(isset($_POST)) {
	...
	$sql = "SELECT email FROM supervisior WHERE email='$email'";
	$result = $conn->query($sql);

	//if email not found then we can insert new data
	if($result->num_rows == 0) {

			//This variable is used to catch errors doing upload process. False means there is some error and we need to notify that supervisior.
	$uploadOk = true;

	//Folder where you want to save your resume. THIS FOLDER MUST BE CREATED BEFORE TRYING
	$folder_dir = "uploads/list/";
	...
	//This is where your files will be saved so in this case it will be uploads/list/newfilename
	...
	if($uploadOk == false) {
		header("Location: register-supervisiors.php");
		exit();
	}
		...
		if($conn->query($sql)===TRUE) {
			...
			// $subject = "Job Site - Confirm Your Email Address";
